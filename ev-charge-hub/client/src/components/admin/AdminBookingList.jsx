...
const AdminBookingsList = () => {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      ...
      // The backend's getAllBookings expects these as query parameters.
      const apiFilters = { ...filters };
      // Remove empty filter values to not send unnecessary params
      Object.keys(apiFilters).forEach(key => {
        if (!apiFilters[key]) delete apiFilters[key];
      });

      // Use the dedicated admin method from apiMethods for fetching all admin bookings
      ...
      if (response.data.success && response.data.data) {
        ...
        // Fallback for direct 'bookings' field
        responseData = response.data;
      } else {
        // Default structure if response.data is just an array or unexpected format
        responseData = {
      ...
      if (err.response) {
        if (err.response.status === 401 || err.response.status === 403) {
          errorMessage = 'Authentication required or not authorized to view bookings. Redirecting to login.';
          // If 401/403, tokens might be invalid/expired, clear them and redirect
          ...
          navigate('/admin/login');
            ...
            errorMessage = 'Booking list endpoint not found. Please check backend routes.';
  ...
  }, [currentPage, limit, filters, getAuthToken, navigate]); // Added dependencies for useCallback
  ...
  const handleFilterChange = (e) => {
    ...
    setCurrentPage(1); // Reset to page 1 when filters change to get relevant results
  };

  // Handle form submission for search/filter
  const handleSearch = (e) => {
    e.preventDefault(); // Prevent default form submission behavior
    fetchBookings(); // Re-fetch data with new filters
  };
  ...
  const formatDate = (dateString) => {
    try {
      ...
      return format(new Date(dateString), 'MMM dd, yyyy HH:mm'); // Added year for clarity
    } catch (err) {
  ...
  // Determine Tailwind CSS classes for status badges based on booking status
  ...
  // Navigate to individual booking details page
  const viewBookingDetails = (bookingId) => {
    navigate(`/admin/bookings/${bookingId}`); // Ensure this matches your React Router path definition
  ...
  const updateStatus = async (bookingId, newStatus) => {
    try {
      ...
      // Use the dedicated admin method from apiMethods for updating status
      const response = await apiMethods.updateAdminBookingStatus(bookingId, newStatus);

      if (response.data.success || response.status === 200) {
        // Update the booking in the local state to reflect the new status immediately
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 mb-8 overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="flex gap-4 mt-6">
                <button
                    onClick={handleSearch} // Trigger search on click as well
                ...
                <button
                    onClick={() => {
                        ...
                        setCurrentPage(1);
                        // Re-fetch after clearing filters to show all data
                        setTimeout(fetchBookings, 0); // Use setTimeout to ensure state update propagates
                    }}
                    className="inline-flex items-center px-6 py-3 border border-gray-300 text-base font-medium rounded-xl text-gray-700 bg-white hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200"
                >
                    Clear Filters
                </button>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-red-200 p-8 mb-8">
            <div className="flex items-center justify-center">
              ...
              <div className="ml-4 text-center sm:text-left">
                ...
                {!getAuthToken() && ( // Show login button only if no token is found
                  <button
        ...
        {/* Bookings Card Layout (only render if not loading and no error) */}
        {!loading && !error && (
          <>
            {bookings.length === 0 ? (
